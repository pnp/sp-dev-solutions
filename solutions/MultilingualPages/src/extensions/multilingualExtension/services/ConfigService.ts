    ...
    try {
      ...
      if (result && result.field) {
        ...
        if (fieldLink) {
          ...
          await sp.web.lists.getById(this._sitePagesLibraryId).fields.getById(fieldId).setShowInNewForm(false);
          await sp.web.lists.getById(this._sitePagesLibraryId).fields.getById(fieldId).setShowInEditForm(false);
  ...
  private async createFieldLink(context: ApplicationCustomizerContext, fieldId: string): Promise<boolean> {
    try {
      ...
      if (sitePageCT.Sealed) {
        ...
        response = await context.spHttpClient.post(`${context.pageContext.site.absoluteUrl}/${fixUrl}`, SPHttpClient.configurations.v1, fixOptions);
        if (!response.ok) {
