...
export class WebPartLogger {
    ...
    public static logUsage(context: IWebPartContext, urlsToCheck?: string[]){
        context.spHttpClient.fetch('/SiteAssets/webpart-logging.config',SPHttpClient.configurations.v1,{}).then(response=>{
            response.json().then(data=>{
                ...
                context.httpClient.post(data.endpoint, HttpClient.configurations.v1, {
                    method: 'POST',
                    mode: 'cors',
                    headers: new Headers({
                        'Content-Type': 'application/json',
                        'Cache-Control': 'no-cache',
                        'x-functions-key': data.apiKey
                    }),
